...
#   endif
  ...
  fs::file_type query_file_type(const path& p, error_code* ec)
  {
    ...
    if (attr & FILE_ATTRIBUTE_REPARSE_POINT)
    {
      ...
      return fs::file_type::reparse_file;
    }
...
#     endif
  ...
  bool remove(const path& p, error_code* ec)
  {
    ...
    if (error(type == file_type::none ? tmp_ec.value() : 0, p, ec,
        "boost::filesystem::remove"))
      return false;
...
#   endif
  ...
  path weakly_canonical(const path& p, system::error_code* ec)
  {
    ...
    for (; !head.empty(); --itr)
    {
      ...
      if (error(head_status.type() == fs::file_type::none,
        head, ec, "boost::filesystem::weakly_canonical"))
        return path();
      if (head_status.type() != fs::file_type::not_found)
        break;
