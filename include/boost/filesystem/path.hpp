...
public:
     ...
     *
     * **Effects:** Constructs a copy of \a p.
     *
     ...
     *
     * \note The \a cvt character code conversion facet is unused and provided for historical reasons,
     ...
     *
     * **Effects:** Move-constructs from \a p.
     *
     * **Throws:** Nothing.
     *
     * \param p Path object to move from.
     *
     * \post `*this` is equal to \a p before the operation.
     */
     ...
     * \brief Initializing constructor.
     *
     * **Effects:** Constructs a path from \a source. Uses \a cvt facet to perform character code conversion,
     *              if needed.
     *
     * **Throws:** `std::bad_alloc` on memory allocation failure, `boost::system::system_error` in case
     *             of character code conversion errors.
     *
     * \param source Path source to construct the path from.
     * \param cvt Character code conversion facet.
     *
     * \pre \a source is a valid path source.
     */
    template<
        typename Source
        //! \cond
     ...
     * \brief Initializing constructor.
     *
     * **Effects:** Constructs a path by moving from \a s.
     *
     * **Throws:** Nothing.
     *
     * \param s Path source to construct the path from.
     *
     * \post `*this` is equal to \a s before the operation.
     */
     ...
     * \brief Initializing constructor.
     *
     * **Effects:** Constructs a path from a range of characters denoted by \a begin and \a end. Uses \a cvt
     ...
     * \brief Copy assignment.
     *
     * **Effects:** Copies the path \a p into `*this`.
     *
     * **Throws:** `std::bad_alloc` on memory allocation failure.
     *
     ...
     * \brief Move assignment.
     *
     * **Effects:** Move-assigns the path \a p to `*this`.
     *
     * **Throws:** Nothing.
     *
     * \param p Path to assign.
     ...
     * \brief Move-assigns characters in the string to the path.
     *
     * **Effects:** Move-assigns the string \a s to `*this`.
     *
     * **Throws:** Nothing.
     *
    ...
    /*!
     * \brief Assigns characters in \a source to the path.
     *
     ...
     * \brief Assigns characters in \a source to the path.
     *
     * **Effects:** Assigns \a source to `*this`. Uses \a cvt facet to perform character code conversion,
     *              if needed.
     *
     * **Throws:** `std::bad_alloc` on memory allocation failure, `boost::system::system_error` in case
     *             of character code conversion errors.
     ...
     * \brief Assigns characters in the iterator range to the path.
     *
     * **Effects:** Assigns a range of characters denoted by \a begin and \a end to `*this`. Uses \a cvt
     *              facet to perform character code conversion, if needed.
     *
     * **Throws:** `std::bad_alloc` on memory allocation failure, `boost::system::system_error` in case
     *             of character code conversion errors.
    ...
    /*!
     * \brief Concatenates character \a c to the end of the path.
     *
     * \returns `concat(&c, &c + 1)`.
     *
     * \pre \c Char is a path character type.
     */
    ...
    /*!
     * \brief Concatenates characters from \a p to the end of the path.
     *
     * \returns `concat(p)`.
     */
    path& operator+=(path const& p);
     ...
     * \brief Concatenates characters from \a p to the end of the path.
     *
     * **Effects:** Copies the path \a p to the end of `*this`.
     *
     * **Throws:** `std::bad_alloc` on memory allocation failure.
     *
    ...
    /*!
     * \brief Concatenates characters from \a source to the end of the path.
     *
    ...
    /*!
     * \brief Concatinates characters from \a source to the end of the path.
     *
     ...
     * \brief Concatinates characters from \a source to the end of the path.
     *
     * **Effects:** Concatenates the characters from \a source to the end of `*this`. Uses \a cvt facet
     ...
     * \brief Concatinates characters from the iterator range to the end of the path.
     *
     * **Effects:** Concatenates a range of characters denoted by \a begin and \a end to the end of `*this`.
     *              Uses \a cvt facet to perform character code conversion, if needed.
     *
     * **Throws:** `std::bad_alloc` on memory allocation failure, `boost::system::system_error` in case
     *             of character code conversion errors.
     *
    ...
    /*!
     * \brief Appends \a p to the end of the path.
     *
     * \returns `append(p)`.
     */
    path& operator/=(path const& p);
     ...
     * **Effects:**
     *
     * **v3:** Concatenates `path::preferred_separator` to `*this`, converting format and encoding if required, unless:
     *
     ...
     *
     * **v4:** If `p.is_absolute() || (p.has_root_name() && p.root_name() != this->root_name())`, assigns `p` to `*this`.
    ...
    /*!
     * \brief Appends \a source to the end of the path.
     *
     ...
     *
     * **Effects:** If `*this` ends with a directory separator, removes that separator. Otherwise, keeps
     *              `*this` unmodified.
     *
     * \returns `*this`.
     */
     ...
     * \returns `*this`.
     *
     * **See Also:** \ref path::remove_filename, \ref path::append.
     */
     ...
     *   \li iff \a new_extension is not empty and does not begin with a dot character, a dot character
     *       is concatenated to the stored path, then
     *   \li \a new_extension is concatenated to the stored path.
     *
    ...
    /*!
     * \brief Swaps `*this` and \a rhs path objects.
     *
     * **Throws:** Nothing.
     *
     ...
     *
     * **See Also:** \ref path::root_path, \ref path::root_name, \ref path::root_directory.
     */
     ...
     *          encoding conversion using \a cvt.
     */
    template< typename String >
     ...
     *          character encoding conversion using \a cvt.
     */
    template< typename String >
    ...
    /*!
     * \returns A value less than 0 if the elements of `*this` are lexicographically less than the elements of \a p,
     *          otherwise a value greater than 0 if the elements of `*this` are lexicographically greater than
     *          the elements of \a p, otherwise 0.
     *
    ...
    /*!
     * **Overview:** Returns `*this` with redundant current directory ("."), parent directory (".."), and
    ...
    /*!
     * **Overview:** Returns `*this` made relative to \a base. Treats empty or identical paths as corner cases,
     *               not errors. Does not resolve symlinks. Does not first normalize `*this` or \a base.
     *
     * \remark Uses `std::mismatch(begin(), end(), base.begin(), base.end())` to determine the first mismatched
     *         element of `*this` and \a base. Uses `operator==()` to determine if elements match.
     *
     ...
     *       to `*this`, \a base, or both.
     *
     * **See Also:** \ref path::lexically_relative.
     */
    ...
    /*!
     * **Effects:** Stores a copy of \a loc as the imbued `path` locale.
     *
 ...
 * \returns A hash value for the path \a p.
 */
