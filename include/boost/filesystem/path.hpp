...
public:
     ...
     * - iff `new_extension` is not empty and does not begin with a dot character, a dot character
     ...
     * \returns
     * \parblock
     * - `path()` if the first mismatched element of `*this` is equal to `begin()` or the first mismatched
     *   element of `base` is equal to `base.begin()`, or
     * - `path(".")` if the first mismatched element of `*this` is equal to `end()` and the first mismatched
     *   element of `base` is equal to `base.end()`, or
     * - An object of class `path` composed via application of `operator/=(path(".."))` for each element in
 ...
 *
 * - A double-quote.
 * - Each character in `p.string< std::basic_string< Char > >()`. If the character to be inserted
 ...
 *   - `p.clear()`.
 *   - Until an unescaped double-quote character is reached or `is.not_good()`, extract characters from `is` and
 *     concatenate them to `p`, except that if an escape character '&' is reached, ignore it and concatenate
 *     the next character to `p`.
