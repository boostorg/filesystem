...
/*!
 * \brief Returns `current_path()` as of the first call to `initial_path`.
 *
 * \note This function is not thread safe and may return an undesirable result if called subsequent to a change
 ...
 *
 * \note The implementation will obtain the required randomness via a cryptographically secure pseudo-random
 *       number generator, such as one provided by the operating system, if possible. Such generators may block until
 *       sufficient entropy develops.
 *
 ...
 *
 * \remark The implementation is allowed (and encouraged) to avoid unnecessary normalization such as when `canonical()`
